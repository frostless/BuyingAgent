...
import { getAllPostsAsyn } from '../services/FetchManage'
import AddPost from './AddPost'
import renderIf from '../services/RenderIf'

export default class PostsDetailsView extends Component {
        ...
        token(this.props.navigation).then((token) => {
            ...
            return fetch(`${apiURL}/api/delete/`, {
                ...
                if (response.status == 200) {
                    let postsNew = this.state.posts;
                    postsNew.splice(index, 1);
                    this.setState({ posts: postsNew }, () => this.forceUpdate());
                }
    ...
    OnGoBack(postNew) {
        ...
        postsNew[this.state.indexForEdit] = postNew;
        this.forceUpdate();
    }
            ...
            <View key={item.id} style={{ flex: 0, height: 50, justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row', borderBottomWidth: 1 }}>
                ...
                <TouchableHighlight
                    ...
                    onPress={() => {
                        ...
                        this.props.navigation.navigate('EditPost', objectToSend);
                    }
                    }
                >
